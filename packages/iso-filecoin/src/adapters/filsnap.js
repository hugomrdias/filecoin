...
/**
 * Create account
 * @param {FilsnapAdapter} adapter
 * @param {string} path
 * @returns {Promise<IAccount>}
 */
async function createAccount(adapter, path) {
  const accountResult = await adapter.getPublicKey()
  if (accountResult.error) {
    throw new Error(accountResult.error.message)
  }
  const publicKey = hex.decode(accountResult.result)

  if (!adapter.config) {
    throw new Error('Filsnap Adapter config not found')
  }
  return {
    type: 'SECP256K1',
    address: AddressSecp256k1.fromPublicKey(publicKey, adapter.config.network),
    publicKey: publicKey,
    path,
  }
}
...
export class WalletAdapterFilsnap extends TypedEventTarget {
    ...
    try {
      ...
      this.#connector = createConnector({
        ...
        onChainChanged: (chain) => {
          const network = this.#connector?.chainIdtoNetwork(chain)
          if (network) {
  ...
  async changeNetwork(network) {
    if (this.network !== network) {
      if (this.filsnap && this.#connector) {
        try {
          const changeChainResult = await this.filsnap.changeChain(network)
  ...
  async deriveAccount(_index) {
    ...
    if (this.#index !== _index) {
      try {
        await this.filsnap?.configure({
          network: this.network,
          derivationPath: pathFromNetwork(this.network, _index),
        })
        this.#index = _index
        this.account = await createAccount(
          this.filsnap,
          pathFromNetwork(this.network, _index)
        )
  ...
  async sign(data) {
    ...
    const r = await this.filsnap.signMessageRaw({ message: utf8.encode(data) })
    if (r.error) {
    ...
    return new Signature({
      ...
      data: base64pad.decode(r.result),
    })
  }
